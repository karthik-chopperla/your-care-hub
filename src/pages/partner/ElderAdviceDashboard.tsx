...
export default function ElderAdviceDashboard() {
  ...
  const loadDashboardData = async (partnerId: string) => {
    try {
      const [expertsRes, bookingsRes, notificationsRes] = await Promise.all([
        supabase.from("elder_experts").select("*").eq("partner_id", partnerId),
        supabase.from("partner_bookings").select("*").eq("partner_type", "elder_advice").eq("partner_id", partnerId),
  ...
  const stats = {
    ...
    pendingQuestions: bookings.filter((b) => b.status === "pending").length,
    answeredQuestions: bookings.filter((b) => b.status === "completed").length,
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input id="medicine_type" value={formData.traditional_medicine_type} onChange={(e) => setFormData({ ...formData, traditional_medicine_type: e.target.value })} />
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {bookings.map((booking) => (
                <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    <p className="font-medium">Request #{booking.id.slice(0, 8)}</p>
                  ...
                  <div className="space-x-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleBookingAction(booking.id, "rejected")}>Decline</Button>
                  </div>
                </div>
              ))}
            </div>
